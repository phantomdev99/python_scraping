...
df7 = pd.DataFrame(df6, columns=["SKU", "QTY"])

df8 = df7[df7["SKU"] !='none']

df8.to_excel('C:/Users/' +currentUser+ '/Downloads/powerbody.xls')
...
csv_data = pd.DataFrame(df_csv, columns=["ArticleNo", "ArticleName", "SellableNumberOfItems"])

csv_data = csv_data[csv_data["SellableNumberOfItems"] != 0]
...
result_excel = pd.DataFrame(excel_data,columns=["SKU", "QTY"])
# result_excel = result_excel[result_excel["SKU"] != 'none']
excel_first_obj = []
excel_second_obj = []
...
new_csv_first_object = csv_first_object.tolist()
new_csv_third_object = csv_third_object

print("SKU",excel_first_obj)
print("QTY", excel_second_obj)
    ...
    if csv_first_object[i][0][0] == "P":
        ...
        for j in range(len(result_excel)):
            ...
            if csv_first_object[i] == excel_first_obj[j]:
                ...
                if i == com[0]:
                    # print("+++++++++++++++++",excel_first_obj[j])
                    new_csv_first_object.append(excel_first_obj[j])
                    # print("+++++++++++++++++",len(csv_first_object))
                    new_csv_third_object.append(excel_second_obj[j][0])
...
result_csv_1 = pd.DataFrame(numpy.array(new_csv_first_object), columns=["ArticleNo"])
result_csv_2 = pd.DataFrame(csv_second_object, columns=["ArticleName"])
...
c = result_csv["SellableNumberOfItems"].eq('0')

deduped = result_csv[~(result_csv.duplicated(['ArticleNo'], keep = "first") & c)]

final_result = deduped[~(deduped.duplicated(['ArticleNo', 'SellableNumberOfItems'], keep = "first"))]
PPP = final_result.sort_values('SellableNumberOfItems', ascending=False).drop_duplicates('ArticleNo').sort_index()
# result_csv = result_csv[result_csv["SellableNumberOfItems"] !='0']
PPP.to_csv('C:/Users/'+currentUser+'/Downloads/nutri-stock.csv', index = False)
